 ...
 * - Tool call parts use 'args' property per ToolCallPart interface
 * - Tool result parts use 'result' property per ToolResultPart interface
 * - Empty messages (no text, no parts) should be skipped
 */

import { describe, it as t, expect, beforeEach } from 'vitest';
import { MessageConversionStrategy } from '../strategies/message.js';
import type {
  Content,
  FunctionResponse,
  FunctionCall,
...
describe('MessageConversionStrategy', () => {
  ...
  describe('geminiToVercel', () => {
    ...
    t('tests that function response result contains the response directly', () => {
      const contents: Content[] = [
        {
          role: 'user',
          parts: [
      ...
      const content = result[0].content as VercelContentPart[];
      const toolResult = content[0] as VercelToolResultPart;
      expect(toolResult.result).toEqual({ data: 'test', success: true });
    });

    t(
      'tests that function response with error field passes error directly',
      () => {
        const contents: Content[] = [
          {
            role: 'user',
    ...
    t(
      'tests that function response without response field uses empty object',
      () => {
        const contents: Content[] = [
          {
            role: 'user',
    ...
    t(
      'tests that function call uses args property per SDK ToolCallPart interface',
      () => {
        const contents: Content[] = [
          {
            role: 'model',
        ...
        const content = result[0].content as VercelContentPart[];
        const toolCall = content[0] as VercelToolCallPart;
        // CRITICAL: Must be 'args' per Vercel AI SDK's ToolCallPart interface
