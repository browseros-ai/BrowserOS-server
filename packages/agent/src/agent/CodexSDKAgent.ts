...
import { Codex, type McpServerConfig } from '@browseros/codex-sdk-fork'
...
import { CODEX_SDK_SYSTEM_PROMPT } from './CodexSDKAgent.prompt.js'
 ...
 * - Metadata tracking
 *
 * Key Differences from ClaudeSDKAgent:
 * - Flat event structure (thread.started, turn.started, item.completed, turn.completed)
 * - Thread-based (stateful) vs message-based (stateless)
 * - Break-loop abort pattern vs native AbortController
 * - MCP config passed to startThread() not query()
 *
...
export class CodexSDKAgent extends BaseAgent {
  ...
  constructor(config: AgentConfig, controllerBridge: ControllerBridge) {
    ...
    logger.info('🔧 CodexSDKAgent constructor', { config, controllerBridge })

    // Configure HTTP MCP server connection
    const httpMcpPort = process.env.HTTP_MCP_PORT || '9100'
    const mcpServerConfig: McpServerConfig = {
      url: `http://127.0.0.1:${httpMcpPort}/mcp`
    } as any

    logger.info(`✅ Configured HTTP MCP server at http://127.0.0.1:${httpMcpPort}/mcp with ${allControllerTools.length} controller tools`)

    // Pass Codex SDK specific defaults to BaseAgent
    super('codex-sdk', config, {
      systemPrompt: CODEX_SDK_SYSTEM_PROMPT,
      mcpServers: { 'browseros-controller': mcpServerConfig },
      maxTurns: CODEX_SDK_DEFAULTS.maxTurns
    })

    logger.info('✅ CodexSDKAgent initialized with HTTP MCP server')
  }
  ...
  override async init(): Promise<void> {
    ...
    if (configUrl) {
      ...
      try {
        ...
        // Initialize Codex instance with system binary
        this.codex = new Codex({ codexPathOverride: '/opt/homebrew/bin/codex' })
        return
      } catch (error) {
  ...
  async destroy(): Promise<void> {
    ...
    // Kill Codex subprocess immediately
    if (this.codex) {
      logger.debug('🛑 Killing Codex subprocess (SIGKILL)')
      this.codex.kill()
    }

    // Also trigger abort controller for cleanup
    if (this.abortController) {
